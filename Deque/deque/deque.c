...
void *get(deque *d, int index)
{
    /// @todo Implement changing the return value!
    node* current = d->head;
    if (is_empty(d) || index > d->size - 1 || index < 0) {
      return NULL;
    } 
...
deque* copy_deque(deque *d, deque_copy copy_func)
{
  ...
  deque* new = create_deque();
  node* current = d->tail;
  while(current) {
    push_front(new, copy_func(current->data));
