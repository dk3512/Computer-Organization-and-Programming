...
public class HW2Operations
{
	...
	{
		//which << 2 to find nibble place
		//~(0xF << (which << 2)) binary mask turns nibble to 0000
		//num & ~(0xF << (which << 2)) clears bit from nibble place
		//(a_nibble) << (which << 2) nibble in position of which
		return num & ~(0xF << (which << 0x2)) | (a_nibble) << (which << 0x2);
	}
